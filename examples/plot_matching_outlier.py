...
def generate_data(nsample, nout, noise, normalize=False):
    ...
    # Generate weights
    if normalize:
        a, b = np.ones(x.shape[0]) / x.shape[0], \
               np.ones(y.shape[0]) / y.shape[0]
    else:
        a, b = np.ones(x.shape[0]) / x.shape[0], \
               np.ones(y.shape[0]) / x.shape[0]
    return a, x + noise * np.random.normal(size=x.shape), \
           b, y + noise * np.random.normal(size=y.shape)


def plot_density_matching(pi, a, x, b, y, ids, alpha, linewidth, fontsize,
                          ftitle='', fname=None):
    n1, n2 = b.shape[0], a.shape[0] - b.shape[0]
    marg1, marg2 = np.sum(pi, axis=1), np.sum(pi, axis=0)
    plt.figure(figsize=(6., 6.))
    plt.scatter(x[:n1, 0], x[:n1, 1], c='b', s=(marg1 / a)[:n1] ** 2 * 25.,
                zorder=1)
    plt.scatter(x[n1:, 0], x[n1:, 1], c='c', s=(marg1 / a)[n1:] ** 2 * 25.,
                zorder=1)
...
if __name__ == '__main__':
    ...
    ids = np.concatenate((np.arange(start=0, stop=nsample, step=10),
                          np.arange(start=nsample, stop=nsample + nout,
                                    step=10)))

    for noise in [0.1]:
        print(f"NOISE = {noise}")
        a, x, b, y = generate_data(nsample=nsample, nout=nout, noise=noise,
                                   normalize=True)
        ...
        if compute_balanced:
            ...
            plot_density_matching(pi_b, a, x, b, y, ids, alpha=1.,
                                  linewidth=0.5, fontsize=16,
        ...
        for rho in [0.01, 1.]:
            ...
            for eps in [1., 0.1, 0.01, 0.001]:
                ...
                PI = log_ugw_sinkhorn(A, CX, B, CY, init=PI, eps=eps,
                                      rho=rho, rho2=rho,
                                      nits_plan=3000, tol_plan=1e-5,
                                      nits_sinkhorn=3000, tol_sinkhorn=1e-5)
            print(f"Mass of transport plan = {PI.sum().item()}")

            # Plot matchings between measures
            pi = PI.cpu().data.numpy()
            plot_density_matching(pi, a, x, b, y, ids, alpha=1., linewidth=1.,
                                  fontsize=16,
