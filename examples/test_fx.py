...
a, Cx, _ = generate_measure(n_batch=1, n_sample=n_sample, n_dim=3,equal=True)
b, Cy, _ = generate_measure(n_batch=1, n_sample=n_sample, n_dim=2,equal=True)
...
for perm in itertools.permutations(range(n_sample),n_sample):
    ...
    if cost < temp:
        ...
        plan = pi_
#print(plan)
print("best cost on permutations: ",cost)
