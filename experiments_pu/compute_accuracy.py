...
if __name__ == '__main__':
    ...
    list_tasks = list_tasks + [(data_pos, data_unl, n_pos, n_unl, prior, nb_try)
    ...
    colnames = ['data_p', 'data_u', 'prior', 'eps', 'rho', 'rho2'] + [str(i) for i in range(nb_try)]

    for (data_pos, data_unl, n_pos, n_unl, prior, nb_try) in list_tasks:
        df = pd.DataFrame(columns=colnames)
        for (eps, rho, rho2) in grid_params:
            # Load Data
            fname = f'/ugw_plan_{data_pos}_{n_pos}_{data_unl}_{n_unl}_prior{prior}_eps{eps}_rho{rho}_rho{rho2}_reps{nb_try}.npy'
            ...
            for i in range(nb_try):
                _, _, y_u = draw_p_u_dataset_scar(data_pos, data_unl, n_pos, n_unl, prior, i)
                # Build prediction
                nu = pi[i]
        ...
        df.to_csv(path + f'/perf_{data_pos}_{n_pos}_{data_unl}_{n_unl}_prior{prior}.csv')
        del df
    print('end')